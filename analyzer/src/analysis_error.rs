...
impl fmt::Display for AnalysisError {
	fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
		...
		match &self.kind {
			...
			UndefinedTypeInMethodDef(typ) => {
				write!(f, "Cannot define method on undefined type {}.", typ)
			}
			...
			UndefinedTypeConstructor(name) => {
				write!(f, "Type constructor '{}' is not defined.", name)
			}
			...
			CalleeNotCallable(typ) => {
				write!(f, "Cannot call value of type {} like a function.", typ)
			}
