					...
					match &variant.kind {
						...
						EnumVariantKind::Constructor(constructor_node, param_node) => {
							...
							let constructor_type =
								ValueType::Func(Box::new(param_type), Box::new(typ.clone()));
			...
			PatternKind::Constructor(ident, param_pattern) => {
				...
				let (param_type, constructor_type) = match &existing_binding.typ {
					ValueType::Func(param_type, return_type) => {
						(param_type.clone(), (**return_type).clone())
					}
					_ => return,
				};
				...
				match &property.kind {
					...
					ExprKind::MultiPartIdentifier { parts } => {
						...
						if let Some(method_type) =
							receiver_type_binding.methods.get(&method_name_parts)
						{
			...
			ExprKind::Interpolation { parts } => {
				...
				for part in parts {
					if !self.compatible_types(&string_type, &part.typ) {
						self.error(AnalysisError {
							pos: part.pos,
							kind: AnalysisErrorKind::TypeMismatchInStringInterpolation(
								part.typ.clone(),
							),
						})
					}
