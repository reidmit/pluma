...
- `wasm/` — the **WasmGC backend** (Pluma's deploy backend): lowers the same `ir::IrProgram` into a WasmGC module, `wasm::emit(&program) -> Result<Vec<u8>, _>`. Covers the full synchronous + async core plus `core.io` under `Platform::Server` (192/193 `tests/run` fixtures match the VM, 0 diffs; the lone gap is a VM-only runtime-error negative test). Capability gating per deploy target lives in `compiler::Platform` (see "Platform targets" below). Correctness is pinned by the `conformance` crate, which runs each emitted module in wasmtime and diffs stdout+status against the VM (194/195 `tests/run` fixtures; the lone skip is a VM-only unknown-builtin negative test).
- `js/` — the **JS backend** (Pluma's browser/client deploy backend): lowers the same `ir::IrProgram` into a self-contained JavaScript module, `js::emit(&program) -> Result<String, _>` (runtime preamble `js/src/runtime.js` + compiled functions + globals + entry). The IR is ANF and JS is uniformly boxed like the VM, so it's a near-1:1 translation (mirrors `codegen`'s traversal; `Repr`/`Box`/`Unbox` are no-ops). Synchronous core today (async `Await`/CPS, the `wire` codec, and full i64 via BigInt are follow-ons); ints are JS `number` (53-bit). Correctness is pinned by the `conformance` crate, which runs each emitted module under `node` and diffs stdout+status against the VM (158/195 `tests/run` fixtures, 0 diffs; `core.io` (gated) and async (unsupported) fixtures are skipped, and a small `denied` list tracks the rest — see `CONFORMANCE.md`). `core.dom` (browser host: `dom.set-text`/`set-html`/`get-value`) is the first browser host surface; see `web/` for a DOM demo (`just web-demo`).
...
- `tests/` — VM-only snapshot tests (analyzer + VM run output) plus the IR tests. The Cargo package is named `tests`. Harness files live at the crate root (`tests/analyze.rs`, `tests/run.rs`) next to the fixture directories (`tests/analyze/`, `tests/run/`). Cross-backend correctness (WasmGC + JS vs the VM oracle) lives in the `conformance/` crate, not here. See "Testing" below.
- `conformance/` — the **cross-backend conformance + perf harness** (the standing reference for all three runtimes). Runs each `tests/run` fixture through the VM (oracle), WasmGC (in wasmtime), and JS (under node), asserts the deploy backends match the VM, collects timing (the compile-vs-exec breakdown), and renders the report. `cargo test -p conformance` is the correctness gate; `cargo run -p conformance` (`just conformance`) regenerates the committed `CONFORMANCE.md` coverage doc; `--perf` adds perf tables → `target/conformance/report.md`. Backend runners: `src/{wasm_host,js_host,run}.rs`; support matrix + corpus in `src/lib.rs`.
