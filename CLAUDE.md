...
- `wasm/` — the **WasmGC backend** (Pluma's sole deploy backend — server/compute now, and the browser/client target as the frontend lands): lowers the same `ir::IrProgram` into a WasmGC module, `wasm::emit(&program) -> Result<Vec<u8>, _>`. Covers the full synchronous + async core plus `core.io` under `Platform::Server`. Capability gating per deploy target lives in `compiler::Platform` (see "Platform targets" below). Correctness is pinned by the `conformance` crate, which runs each emitted module in wasmtime and diffs stdout+status against the VM — **full parity: 216/216 execution fixtures (`tests/run` + `tests/run-fail`) match, 0 diffs, 0 skips** (VM-internal negative tests like the unknown-builtin guard live in `tests/vm_errors.rs`, not the cross-backend corpus).
