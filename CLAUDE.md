...
Pluma has **one backend**: the **WasmGC** backend. `pluma run` and `pluma test` both compile to WasmGC and run under V8 via the `host` crate — run what you deploy. Every builtin lowers to wasm; the only surface the backend can't emit is the browser-only `core.dom` (unbuilt). "Server" and "browser" are host-capability *profiles* (`compiler::Platform`), not separate backends.
...
- **Matching:** refutable `if subj is pat { ... } else { ... }` (the `else` is the no-match case). The `is pat` is **optional** and defaults to `is true`, so `if x > 10 { ... }` is a plain boolean condition; a `{` after the subject always opens the body, so parenthesize a subject that must end in a record (`if (f { ... }) { ... }`). Same for `while`. `when c is red { ... } is green { ... }` chains arms — only the first carries the subject, and each arm still needs its `is pat`. `when` is exhaustiveness-checked.
...
- **Imports: `use core.foo`.** `ref`/`option`/`result` are auto-imported; `core.task` is **not** — `use core.task` to name any `task.*` function.
- **Tests are a library, not syntax.** A `*.test.pa` file exports `def tests :: test.suite = [ test.case "name" (fun { ... }), ... ]` built from `core.test`, with assertions from `core.assert`. `pluma test` discovers and runs them under V8.
