...
This repo (named `pencil` on disk) implements **Pluma**, a small statically-typed functional language. The CLI binary is named `pluma`, source files use the `.pa` extension. The language is documented under `site/content/docs/` (the zola-built docs site). Pluma has two execution backends sharing one IR: the **VM** (`pluma run`/`pluma test`/tests/bench — the reference interpreter, not a deploy target) and the **WasmGC** backend (the deploy artifact; see "Workspace layout" and "Platform targets").
...
- `vm/` — bytecode VM that executes the compiled program. `VM::new(program).run()` is the entry point. `print` writes through a configurable `StdoutSink` (process stdout by default; tests inject a `Buffer` sink). `vm::stdlib::register_compiler` seeds the analyzer with any Rust-defined native module types — currently none, since every stdlib module (including `core.dict`) is a `.pa` source; the mechanism remains for any future module whose signature the `.pa` surface can't express. **Charter:** the VM is Pluma's *reference interpreter* and the *dev/test/bench engine* (`pluma run`/`pluma test`, the `tests/run` differential oracle, `bench`) — it is **not** a deploy target. The WasmGC backend is the only deploy artifact; the VM corresponds to `Platform::Native`, which provides every capability so nothing is ever gated. Keep the two in lockstep: it's the cross-check (`tests/wasm_diff`) that catches WasmGC codegen bugs.
- `wasm/` — the **WasmGC backend** (Pluma's deploy backend): lowers the same `ir::IrProgram` into a WasmGC module, `wasm::emit(&program) -> Result<Vec<u8>, _>`. Covers the full synchronous + async core plus `core.io` under `Platform::Server` (192/193 `tests/run` fixtures match the VM, 0 diffs; the lone gap is a VM-only runtime-error negative test). Capability gating per deploy target lives in `compiler::Platform` (see "Platform targets" below). Correctness is pinned by `tests/wasm_diff.rs`, which runs each emitted module in wasmtime and diffs stdout+status against the VM.
...
- `tests/` — integration tests (snapshot-based) for the analyzer and the VM, plus the WasmGC differential (`tests/wasm_diff.rs`: emit each `tests/run` fixture to WasmGC, run it in wasmtime, diff stdout+status against the VM — the VM is the oracle). The Cargo package is also named `tests`. Harness files live at the crate root (`tests/analyze.rs`, `tests/run.rs`) next to the fixture directories (`tests/analyze/`, `tests/run/`). See "Testing" below.
