...
- **Matching:** refutable `if subj is pat { ... } else { ... }` (the `else` is the no-match case); `when c is red { ... } is green { ... }` chains arms — only the first carries the subject. `when` is exhaustiveness-checked.
