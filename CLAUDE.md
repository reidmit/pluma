...
- **String interpolation `"$(expr)"`** needs explicit `to-string` for non-strings: `"n = $(to-string n)"`. Only `$(` interpolates; write a literal one by escaping the dollar as `\$(`.
...
- **Tests are a library, not syntax.** A `*.test.pa` file exports `def tests :: test.suite = [ test.case "name" (fun { ... }), ... ]` built from `std/test`, with assertions from `std/assert`. `pluma test` discovers and runs them under V8. A test file sees the **private** surface of the module it tests — `foo.test` importing `foo` resolves `foo`'s private defs (values, enums + constructors, opaque-enum constructors, aliases, traits) as if public, so unit-test a helper directly instead of marking it `public` just for the test. Only the sibling whose name matches gets this; every other importer sees the public surface.
...
just test-run           # the tests/pa suite alone (pluma test, isolate per module)
