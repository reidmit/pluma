...
- `conformance/` — the **cross-backend conformance harness** (the standing correctness reference for both runtimes). Runs each execution fixture (`tests/run` ∪ `tests/run-fail`) through the VM (oracle) and WasmGC (under V8, via the shared `host` crate), asserts WasmGC matches the VM, and regenerates the committed `CONFORMANCE.md` coverage doc. `cargo test -p conformance` is the correctness gate; `cargo run -p conformance` (`just conformance`) rewrites `CONFORMANCE.md`. The WasmGC runtime lives in the shared `host` crate (the same one `cli` ships); the VM oracle + shared compile step are `src/run.rs`; support matrix + corpus in `src/lib.rs`. (Perf is measured elsewhere — `competition/` for process-level deploy timing, `bench/` for VM compute throughput.)
- `bench/` — microbench runner that times each `bench`-marked `tests/run/<name>/` fixture through the VM. Benchmarks live in `tests/run` (so they also get snapshot + cross-backend correctness coverage); an empty `bench` marker file in the fixture dir flags the compute-stress subset that `bench` selects on.
...
The `conformance` crate's cross-backend correctness corpus is `tests/run` ∪ `tests/run-fail` (the deploy backends must reproduce the VM oracle on both happy and failing runs). Perf is *not* measured here — it lives in `bench/` (VM compute throughput over the `bench`-marked `tests/run` fixtures — the longer stress programs moved over from the former `benchmarks/programs`) and `competition/` (process-level deploy timing vs other languages).
