...
just test <substring>      # filter tests by name substring
...
`Compiler` (in `compiler/src/compiler.rs`) is the orchestration entry point. It owns a `HashMap<String, Module>` and a `Vec<Diagnostic>`, exposing `tokenize()` and `check()`. Both consume an entry path resolved at construction time. There is no `build()`/`run()` backend yet — `check()` runs through parsing + type analysis and returns the typed `Module`.

Pipeline for `check()`:

1. **Tokenize** — `Tokenizer::from_source(&bytes)` (in `tokenizer.rs`) yields `Token`s from raw source bytes.
2. **Parse** — `Parser::new(bytes, tokenizer).parse_module()` produces a `ModuleNode` AST plus a comment map plus `ParseError`s. The AST is split across `ast/*.rs` with one file per node category (`call`, `definition`, `enum`, `expr`, `fun`, `if`, `let`, `literal`, `pattern`, `when`, `while`, etc.) and a shared `ast/mod.rs` re-export list. `module.rs` stores the AST plus line-break offsets on the `Module`.
3. **Analyze** — `Analyzer::analyze(module)` in `compiler/src/analyzer.rs` is a Hindley-Milner-style three-phase pass:
   ...
   Built-in types (`int`, `bool`, `string`, `regex`, `float`) are seeded into the type scope at the start of `analyze`. Value/type scopes are stacks of `HashMap`s; identifiers resolve into `ValueBinding` / `TypeBinding` (in `binding.rs`).
...
- AST node types are split one-per-file under `ast/`; add new nodes by adding a module and re-exporting from `ast/mod.rs`.
