...
This repo (named `pencil` on disk) implements **Pluma**, a small statically-typed functional language. The CLI binary is named `pluma`, source files use the `.pa` extension. The language is documented under `site/content/docs/` (the zola-built docs site). Pluma has **two backends** sharing one IR: the **VM** (`pluma run`/`pluma test`/tests/bench — the reference interpreter and differential oracle, not a deploy target) and the **WasmGC** backend (the single deploy artifact — server/compute today, and the browser/client target as the frontend lands, via a small hand-written JS DOM shim). (A speculative third **JS** backend was retired in favor of WasmGC-for-browser — see `notes/DEPLOY.md`.) See "Workspace layout" and "Platform targets".
...
- `ir/` — lowers the typed AST into a mid-level IR (`ir::lower(&compiler)` returns an `ir::IrProgram`). The IR is where backend-independent optimization (devirtualization, repr/box-unboxing, monomorphization, async CPS) happens, and it's the single source both backends — `codegen` (→ VM bytecode) and `wasm` (→ WasmGC) — consume.
...
- `vm/` — bytecode VM that executes the compiled program. `VM::new(program).run()` is the entry point. `print` writes through a configurable `StdoutSink` (process stdout by default; tests inject a `Buffer` sink). `vm::stdlib::register_compiler` seeds the analyzer with any Rust-defined native module types — currently none, since every stdlib module (including `core.dict`) is a `.pa` source; the mechanism remains for any future module whose signature the `.pa` surface can't express. **Charter:** the VM is Pluma's *reference interpreter* and the *dev/test/bench engine* (`pluma run`/`pluma test`, the `tests/run` differential oracle, `bench`) — it is **not** a deploy target. The VM corresponds to `Platform::Native`, which provides every capability so nothing is ever gated. Keep both backends in lockstep: it's the cross-check (`conformance/`, which diffs WasmGC against the VM oracle) that catches codegen bugs.
- `wasm/` — the **WasmGC backend** (Pluma's sole deploy backend — server/compute now, and the browser/client target as the frontend lands): lowers the same `ir::IrProgram` into a WasmGC module, `wasm::emit(&program) -> Result<Vec<u8>, _>`. Covers the full synchronous + async core plus `core.io` under `Platform::Server`. Capability gating per deploy target lives in `compiler::Platform` (see "Platform targets" below). Correctness is pinned by the `conformance` crate, which runs each emitted module under V8 (the deploy engine) and diffs stdout+status against the VM — **full parity: 218/218 execution fixtures (`tests/run` + `tests/run-fail`) match, 0 diffs, 0 skips** (VM-internal negative tests like the unknown-builtin guard live in `tests/vm_errors.rs`, not the cross-backend corpus).
...
just conformance           # cross-backend conformance: diff WasmGC vs the VM oracle, regen CONFORMANCE.md
just conformance-perf      # + perf tables (VM/WasmGC timing) → target/conformance/report.md
