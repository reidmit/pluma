...
The `conformance` crate's cross-backend correctness corpus is `tests/run` ∪ `tests/run-fail` (the deploy backends must reproduce the VM oracle on both happy and failing runs); its perf dev-loop benches `tests/run` only.
