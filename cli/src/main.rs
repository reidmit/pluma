	...
	match &subcommand.unwrap()[..] {
		"check" => {
			if is_help_requested {
				...
				let mut parsed_args =
					arg_parser::parse_args_for_command(args, CheckCommand::info())?;
		...
		"version" => {
			if is_help_requested {
				...
				let mut parsed_args =
					arg_parser::parse_args_for_command(args, VersionCommand::info())?;
		...
		"help" => {
			if is_help_requested {
				...
				let mut parsed_args =
					arg_parser::parse_args_for_command(args, HelpCommand::info())?;
				HelpCommand::execute(&mut parsed_args)?;
			}
		}
