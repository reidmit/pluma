...
impl Helper {
	...
	/// and in-order.
	pub(crate) const COUNT: usize = 107;
}
...
pub(crate) struct TaskGlobals {
	...
	pub(crate) free_scopes: u32, // mut ref null $value — $list (stack) of recyclable sids: a sid lands here once its scope finalized (body + all children settled), so the slot can back a future scope instead of growing the scopes table forever
...
pub(crate) fn is_io_result(tag: &str) -> bool {
	matches!(
		...
		"io-read"
			...
			| "fs-op-sync"
			// `std/compress` codecs — each returns `result bytes string` via `__io_result`.
			| "gzip-encode"
			| "gzip-decode"
			| "brotli-encode"
			| "brotli-decode"
