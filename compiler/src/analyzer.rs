...
// The complete set of variants that stay reachable *bare* in every module —
// the one deliberate exception to qualified `enum.variant` access. Limited to
// the four constructors pervasive in everyday code, each pinned to its prelude
// enum: a user enum that reuses one of these names (e.g. its own `result` with
// `ok`/`err`) is *not* matched, so it stays qualified like any other. Everything
// else (the prelude's own `ordering`/`wire-error`/`wire-schema`/`regex-pattern`/
// `request` variants included) is written `enum.variant`.
fn is_bare_prelude_variant(qualified_enum: &str, variant: &str) -> bool {
	matches!(
		(qualified_enum, variant),
		("__prelude__.option", "some" | "none") | ("__prelude__.result", "ok" | "err")
	)
}
...
pub struct Analyzer<'compiler> {
	...
	remote_endpoints: Vec<(Range, String, Type)>,
	// Validated `remote def` endpoint metadata (resolved wire shapes + per-route
...
impl<'compiler> Analyzer<'compiler> {
	...
	pub fn analyze(&mut self, module: &mut Module) {
		...
		// the substitution has bound each captured annotation's vars to concrete
		// types, so the argument and result shapes are visible. Draining keeps
		// this from leaking across modules.
		for (range, name, ty) in std::mem::take(&mut self.remote_endpoints) {
	...
	fn constrain(&mut self, module: &mut ModuleNode) -> Vec<Constraint> {
		...
		for definition in &mut module.body {
			...
			// mandatory and must shape up as `fun A.. -> task R`. The resolved
			// annotation already has any `task`/`list` expanded, so checking it
			// here (rather than the inferred body type) sees concrete argument
			// and result types.
			if definition.is_remote {
				match &annotated_ty {
					Some(ty) => self.remote_endpoints.push((
						definition.name.range,
						definition.name.name.clone(),
						ty.clone(),
					)),
				...
				PatternKind::Identifier(ident) => {
					...
					let is_nullary_variant = match subject_ty {
						Type::Enum(enum_name, _) => {
							is_bare_prelude_variant(enum_name, &ident.name)
								&& self
	...
	fn pattern_is_catch_all(&self, pattern: &PatternNode) -> bool {
		match &pattern.kind {
			...
			PatternKind::Identifier(ident) => self
				...
				.iter()
				.filter(|(q, _)| is_bare_prelude_variant(q, &ident.name))
				.all(|(_, p)| !p.is_empty()),
		...
		if let Type::Enum(enum_name, _) = subject_ty {
			return match self.find_variant_in_enum(enum_name, &name.name) {
				Some(params) => {
					...
					if is_bare_prelude_variant(enum_name, &name.name) {
	...
	// Drains the validated `remote def` metadata accumulated during `analyze`.
	// Called by the compiler after each module is analyzed; the lowerer reads
	// the collected shapes to synthesize client stubs and the dispatch table.
	pub fn take_endpoint_meta(&mut self) -> Vec<crate::rpc::RpcEndpointMeta> {
		std::mem::take(&mut self.endpoint_meta)
	}

	// Validates a `remote def`'s signature is a well-formed RPC endpoint
	// contract: `fun A1..An -> task R`, the pure logical signature (no transport
	// `request` — context is ambient). Every argument and the task's result `R`
	// must be `wire`-derivable so they can cross the network. On success, records
	// the endpoint's resolved arg/result wire shapes + per-route fingerprint into
	// `endpoint_meta`. Diagnostics are accumulated, never raised.
	fn validate_remote_endpoint(&mut self, range: Range, name: &str, ty: &Type) {
		let Type::Fun(params, ret) = ty else {
			self.error(
				range,
				AnalysisErrorKind::RemoteDefSignature {
					detail: "it must be a function (the signature is the contract)".to_string(),
				},
			);
			return;
		};
		...
		// must be `wire`-derivable, with attribution when it isn't (the same
		// boundary diagnostic `wire` uses everywhere). Collect the arg shapes as
		// we go so a clean signature also yields the metadata the lowerer needs.
		let mut ok = true;
		let mut arg_shapes: Vec<WireShape> = Vec::with_capacity(params.len());
		let mut boundary: Vec<&Type> = params.iter().collect();
		boundary.push(&args[0]);
		for t in &boundary {
			match self.build_wire_shape(t, &mut Vec::new()) {
				Some(shape) => arg_shapes.push(shape),
				None => {
					ok = false;
					let detail = self
		...
		// The result shape is the last entry we pushed; the rest are the args.
		let result_shape = arg_shapes.pop().unwrap();
		// The encode/decode shape for the arguments: the single shape for arity
		// 1, a `Tuple` of them for arity ≥ 2, an empty `Tuple` for arity 0
		// (nothing is sent). Encode and decode share it.
		let arg_shape = if arg_shapes.len() == 1 {
			arg_shapes.pop().unwrap()
		} else {
