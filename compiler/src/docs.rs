...
/// Render one or more module docs as a self-contained Pluma data module: a few
/// record-type aliases plus a `public def modules` literal. The website `use`s
/// it and renders the data, so stdlib docs come straight from source and stay
/// honest — the literal even type-checks against the aliases.
pub fn to_pluma_source(modules: &[ModuleDoc]) -> String {
	let mut body = String::new();
	body.push_str("public def modules :: list doc-module = [\n");
	for m in modules {
		body.push_str("\t{\n");
		body.push_str(&format!("\t\tname: {},\n", lit(&m.name)));
		body.push_str(&format!("\t\tnamespace: {},\n", lit(&m.namespace)));
		body.push_str("\t\tblocks: ");
		push_blocks(&mut body, &m.blocks, 2);
		body.push_str(",\n");
		body.push_str("\t\titems: [\n");
		for it in &m.items {
			body.push_str("\t\t\t{\n");
			...
			body.push_str("\t\t\t\tblocks: ");
			push_blocks(&mut body, &it.blocks, 4);
			body.push_str(",\n");
			body.push_str("\t\t\t},\n");
		}
		body.push_str("\t\t],\n");
	...
	out.push_str("# Generated by `pluma doc`. Do not edit by hand — regenerate from source.\n\n");
	out.push_str("public alias doc-span {\n\tclass :: string\n\ttext :: string\n}\n\n");
	out.push_str(
		"public alias doc-block {\n\tkind :: string\n\ttext :: string\n\tspans :: list doc-span\n}\n\n",
	);
	out.push_str(
		"public alias doc-item {\n\tname :: string\n\tsignature :: string\n\tblocks :: list doc-block\n}\n\n",
	);
	out.push_str(
		"public alias doc-module {\n\tname :: string\n\tnamespace :: string\n\tblocks :: list doc-block\n\titems :: list doc-item\n}\n\n",
	);
	// Doc text can contain `$(`, which a Pluma string literal would read as
	// interpolation; `lit` splices the `$` in via this def instead. Only emit it
	// when something used it, to avoid an unused-binding warning.
	if body.contains(" ++ dol ++ ") {
		out.push_str("def dol :: string = \"$\"\n\n");
	}
	out.push_str(&body);
	out
}

fn push_blocks(out: &mut String, blocks: &[DocBlock], indent: usize) {
	if blocks.is_empty() {
		out.push_str("[]");
		return;
	}
	...
	for b in blocks {
		out.push_str(&format!(
			"{pad1}{{kind: {}, text: {}, spans: ",
			lit(b.kind),
			lit(&b.text)
		));
		push_spans(out, &b.spans);
		out.push_str("},\n");
	}
	out.push_str(&pad);
	out.push(']');
...
// A code block's highlighted spans, inline: `[{class: "keyword", text: "def"}, …]`.
fn push_spans(out: &mut String, spans: &[DocSpan]) {
	if spans.is_empty() {
		out.push_str("[]");
		return;
	}
...
/// A Pluma string expression for `s`. Usually a plain literal, but a `$(` —
/// which the tokenizer would treat as interpolation — is broken by splicing the
/// `$` in through the `dol` def, so `"a $(b)"` becomes `"a " ++ dol ++ "(b)"`.
/// A lone `$` (not before `(`) is a safe literal and left alone.
fn lit(s: &str) -> String {
	...
	while let Some(c) = chars.next() {
		match c {
			...
			'\t' => out.push_str("\\t"),
			'\r' => out.push_str("\\r"),
			'$' if chars.peek() == Some(&'(') => out.push_str("\" ++ dol ++ \""),
			_ => out.push(c),
		}
	}
	out.push('"');
