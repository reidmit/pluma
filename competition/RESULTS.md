...
_Best of 5 runs, wall-clock seconds (lower is better). Generated 2026-06-03 10:16:33 PDT._
...
|---|---|
| host | `Darwin arm64` |
| pluma | git `a99878a` (release build) |
...
| `sort` | sort + checksum | 0.07 | 0.04 | 0.08 | 0.06 | 1.8x | ok |
| `dict` | hash-map tally | 0.10 | 0.06 | 0.09 | 0.04 | 2.5x | ok |
| `string` | join / split / upcase | 0.04 | 0.02 | 0.07 | 0.03 | 2.0x | ok |
| `tree` | build + fold a tree | 0.53 | 3.29 | 0.96 | 0.11 | 4.8x | ok |
| `collections` | map / filter / fold | 0.11 | 0.12 | 0.16 | 0.14 | 0.9x | ok |

One-time cost to compile all 8 benchmarks to WasmGC artifacts: **0.04s** total (not included in the per-run `pluma-v8` times).
